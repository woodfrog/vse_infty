...
class COCORawHDFSDataset(data.IterableDataset):
    """
    Load precomputed captions and image features
    Possible options: f30k_precomp, coco_precomp
    """

    def __init__(self, data_path, data_split, tokenzier, opt, train, num_readers, shuffle):
        self.opt = opt
        self.train = train
        ...
        loc_image = osp.join(data_path, 'original_updown')
        loc_mapping = osp.join(data_path, 'original_updown/id_mapping.pkl')

        self.id_to_path = pickle_load(loc_mapping)
        self.image_base = 'hdfs:///home/byte_arnold_hl_vc/user/chenjiacheng/data/packed_coco_images/coco'
        ...
        # Image features
        path = osp.join(loc_image, '{}_rois.pkl'.format(data_split))
        self.images = pickle_load(path)
    ...
    def reset_training_image_scale(self):
        new_scale = random.random() * (200.0 / 512) + (400.0 / 512)
        self.train_scale_rate = new_scale
        logger.info('Set the training image scale to {}'.format(new_scale))
        ...
        for i in range(num_read_iter):
            ...
            for index in all_index_worker[i * self.kv_reader_batch: (i + 1) * self.kv_reader_batch]:
                ...
                if not isinstance(image_path,
                    ...
                    image_path = os.path.join(self.image_base, self.id_to_path[image_path])

                prefix_len = len(self.image_base)
                img_key = image_path[prefix_len:]  # get the key from the saved path in the pickle file
                img_keys.append(img_key)
                all_image_rois.append(image_rois)
    ...
    def _random_rotate(im):
        rand_degree = random.choice([45, 90, 135, 180, 225, 270, 315])
        rot_image = ndimage.rotate(im, rand_degree, reshape=False)
        return rot_image
...
class COCOPrecompHDFSDataset(data.Dataset):
    """
    Load precomputed captions and image features
    Possible options: f30k_precomp, coco_precomp
    """
...
def get_loader(data_path, data_split, tokenizer, opt, batch_size=100,
    ...
    if opt.precomp_enc_type == 'basic':
        dset = COCOPrecompHDFSDataset(data_path, data_split, tokenizer, opt, train)
        ...
        dset = COCORawHDFSDataset(data_path, data_split, tokenizer, opt, train, num_readers, shuffle)
        data_loader = torch.utils.data.DataLoader(dataset=dset,
                                                  batch_size=batch_size,
                                                  num_workers=num_workers,
...
if __name__ == '__main__':
    ...
    # dataset = COCOHDFSDataset(data_path, data_split, vocab, opt, train, num_readers, shuffle=True)
