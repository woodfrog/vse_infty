...
CUDA_VISIBLE_DEVICES=0,1,2,3 python3 train.py \
  ...
  --cross_attn=none --num_epochs=25 --lr_update=15 --learning_rate=.00005 --precomp_enc_type backbone  --workers 20 --backbone_source vsepp_detector \
  ...
  --input_scale_factor 2.0  --backbone_path ${DATA_PATH}/original_updown/original_updown_backbone.pth  --drop  --resume runs/${DATASET_NAME}_vsepp_updown_bert_var_gpool_test/checkpoint.pth.tar
